...
<head>
    ...
    <title>DIGIY POS ELITE — Royal Edition</title>
      <!-- Supabase -->
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Inter', sans-serif; }
        ...
        /* Écran de verrouillage - Tunnel de vente */
...
<div id="lockScreen">
    <div class="lock-box">
        ...
        <h2 style="margin-bottom: 10px;">ESSAI TERMINÉ</h2>
        <p style="color: var(--muted); margin-bottom: 25px; font-size: 14px;">
            Pour continuer à utiliser vos outils de gestion et débloquer les rapports comptables illimités, passez à la version Pro.
        </p>
        
        <a href="https://beauville.github.io/commencer-a-payer/" class="pay-link-btn">
            <i class="fas fa-shopping-cart"></i> COMMENCER À PAYER
        </a>

        <div style="margin: 20px 0; color: var(--muted); font-size: 11px;">OU ENTREZ VOTRE CLÉ PERSONNELLE</div>
...
<div id="adminPage" class="page">
    <div class="main-container" style="display: block; max-width: 600px; margin: auto;">
        <div class="form-card dash-link">
            <h3><i class="fas fa-chart-line"></i> DASHBOARD & COMPTABILITÉ</h3>
        ...
        <div class="form-card">
            ...
            <p style="color:var(--muted); font-size:13px; margin-bottom:15px;">
                Configure tes infos légales, TVA, logo et mentions pour les tickets
            </p>
            <button class="gold-btn" onclick="goSettings()">OUVRIR LES PARAMÈTRES</button>
        </div>        
        <div class="form-card">
            <h3>📦 NOUVEAU PRODUIT</h3>
            <input type="text" id="addName" placeholder="Désignation">
...
(async function init() {
    if (!window.DIGIY_GUARD || typeof window.DIGIY_GUARD.boot !== "function") {
        console.warn("Guard absent (guard.js non chargé).");
        return;
    }
    ...
    console.log("✅ Session OK, owner_id:", State.owner_id);
...
async function checkout(method) {
    ...
    try {
        ...
        await printTicket(State.cart, total, method);
...
// GÉNÉRATION TICKET
// =============================
async function printTicket(cart, total, method) {
    // Charger settings pour TVA et infos
    const sb = getSB();
    const { data: settings } = await sb
        .from('digiy_caisse_settings')
        .select('*')
    ...
    if (s.tva_assujeti && s.tva_taux > 0) {
        totalHT = total / (1 + s.tva_taux / 100);
        totalTVA = total - totalHT;
        totalTTC = total;
    }

    // Générer HTML du ticket
    const ticketHTML = `
<!DOCTYPE html>
<html>
<head>
...
<body>
    ...
    <div class="center bold" style="font-size: 14px;">
        ${s.store_name || 'MA BOUTIQUE'}
    </div>
    
    ${s.address ? `<div class="center" style="font-size: 10px;">${s.address}</div>` : ''}
    ${s.phone ? `<div class="center" style="font-size: 10px;">Tél: ${s.phone}</div>` : ''}
    ${s.siret ? `<div class="center" style="font-size: 10px;">SIRET: ${s.siret}</div>` : ''}
    ...
    ${cart.map(item => `
        ...
        <div style="font-size: 10px; margin-left: 10px;">
            ${item.qty} x ${item.price.toLocaleString()} F
        </div>
    `).join('')}
    ...
    ${s.tva_assujeti ? `
        <div class="row">
            <span>Total HT</span>
            <span>${Math.round(totalHT).toLocaleString()} F</span>
        </div>
        <div class="row">
            <span>TVA (${s.tva_taux}%)</span>
    ...
    <div class="center footer" style="margin-top: 15px; font-size: 8px;">
        Propulsé par DIGIY CAISSE PRO 🦅
    </div>
</body>
</html>
    `;
