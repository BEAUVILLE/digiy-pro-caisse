...
(() => {
  ...
  /* ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
     ...
     new URL(relatif, location.href) fragile selon l'env.
     On reconstruit l'origine manuellement.
     Utilisée par buildPinUrl et buildPayUrl.
  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ */
  function buildSafeUrl(base, params) {
    ...
    try {
      if (/^https?:\/\//i.test(baseStr)) {
        ...
        const full = baseStr.startsWith("/") ? origin + baseStr : origin + "/" + baseStr;
        u = new URL(full);
      }
    } catch(e) {
  ...
  function saveSlugOnly(slug) {
    ...
    if (!clean) return;
    try { localStorage.setItem(CFG.STORAGE.SLUG_KEY, clean); localStorage.setItem(CFG.STORAGE.LAST_SLUG_KEY, clean); sessionStorage.setItem(CFG.STORAGE.SLUG_KEY, clean); sessionStorage.setItem(CFG.STORAGE.LAST_SLUG_KEY, clean); } catch (_) {}
  }
  ...
  function readSavedSlug() {
    try { return normSlug(qs.get("slug") || sessionStorage.getItem(CFG.STORAGE.SLUG_KEY) || sessionStorage.getItem(CFG.STORAGE.LAST_SLUG_KEY) || localStorage.getItem(CFG.STORAGE.SLUG_KEY) || localStorage.getItem(CFG.STORAGE.LAST_SLUG_KEY) || ""); }
  ...
  /* ━━━ buildPinUrl — utilise buildSafeUrl au lieu de new URL(base, location.href) ━━━ */
  ...
  async function resolveSubBySlug(slug) {
    ...
    for (const params of tries) {
      ...
      return { slug: normSlug(res.data[0].slug), phone: normPhone(res.data[0].phone), module: upper(res.data[0].module || MODULE) };
    }
    return null;
  }
  ...
  function parseVerifyPinPayload(data, fallbackPhone = "") {
    ...
    if (typeof raw === "object" && !Array.isArray(raw)) {
      if (raw.ok === true) {
        return { ok: true, phone: normPhone(raw.phone || raw.p_phone || fallbackPhone || ""), module: upper(raw.module || raw.p_module || MODULE), owner_id: raw.owner_id || null };
      }
      const vals = Object.values(raw);
      if (vals.length >= 3) {
        const okLike = vals[0] === true || vals[0] === "t" || vals[0] === "true" || vals[0] === 1;
        if (okLike) return { ok: true, module: upper(vals[1] || MODULE), phone: normPhone(vals[2] || fallbackPhone || ""), owner_id: vals[4] || null };
      }
    }
    ...
    if (typeof raw === "string") {
      ...
      if (txt.startsWith("(") && txt.endsWith(")")) {
        ...
        if (tupleHead) {
          ...
          if (okLike) return { ok: true, module: upper(modToken || MODULE), phone: normPhone(phoneToken || fallbackPhone || ""), owner_id: null };
        }
      }
    }
    return null;
  ...
  async function loginWithPin(slug, pin) {
    ...
    /* ━━━ DOCTRINE FAÇADE : strings d'erreur humaines ━━━
       AVANT : "Slug manquant." / "Slug inconnu (phone non résolu)."
       MAINTENANT : langage humain sans jargon technique */
    ...
    if (!phone) {
      return { ok: false, error: "Identifiant introuvable. Vérifie ton lien pro ou contacte le support." };
    }
  ...
  async function check() {
    ...
    if (!slug && !phone) {
      ...
      clearSessionsOnly();
      state.access = false; state.access_ok = false; state.preview = true;
      state.ready_flag = true; state.error = "Accès non identifié.";
      showPage();
      goPin({ slug, phone });
      return { ...state };
    }
    ...
    if (!freshSession) {
      ...
      state.access = false; state.access_ok = false; state.preview = true;
      state.ready_flag = true;
      /* ━━━ AVANT : "Session PIN absente ou expirée." ━━━ */
